...
use crate::file::{self, scan, write, image};
...
pub fn build(dir: &PathBuf, output_dir: &PathBuf) -> Result<(), std::io::Error> {
    ...
    let document: Vec<MdDocument> = files
        ...
        .filter_map(|mut doc| image::copy_document_images(&mut doc, &images_dir).ok().map(|_| doc))
        .collect();

    println!("{} HTML files are being written to the {:#?} directory.", document.len(), output_dir);

    document
        .iter()
        .map(|doc| md_to_html(doc))
        .try_for_each(|html_doc| -> Result<(), std::io::Error> {
