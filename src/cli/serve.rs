...
async fn html_path_resolver(State(state): State<ServerState>, mut req: Request, next: Next) -> Response {
    let path = req.uri().path();

    let decoded_path = percent_decode_str(path).decode_utf8_lossy();

    let has_extension = decoded_path.split('/').last().map_or(false, |p| p.contains('.'));
    let is_directory = decoded_path.ends_with('/');

    if !has_extension && !is_directory {
        let html_file_path = state.output_dir
